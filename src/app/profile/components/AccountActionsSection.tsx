...
export function AccountActionsSection({ isVerified = false, userType = 'listener' }: AccountActionsSectionProps) {
  const [isVerificationDialogOpen, setIsVerificationDialogOpen] = useState(false);
  const [isCreatorDialogOpen, setIsCreatorDialogOpen] = useState(false);

  return isVerified && userType =='creator'?'':(   
     <motion.div
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        {/* Only show Verify Account if not verified */}
        {!isVerified && (
          <Dialog open={isVerificationDialogOpen} onOpenChange={setIsVerificationDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[425px] max-w-[320px] rounded-md p-7">
              <DialogHeader>
        ...
        {/* Only show Become Creator if user is not already a creator */}
        {userType === 'listener' && (
          <Dialog open={isCreatorDialogOpen} onOpenChange={setIsCreatorDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[445px] max-w-[320px] rounded-md p-4 sm:p-6">
              <DialogHeader>
                <DialogTitle>Become a Creator</DialogTitle>
                <DialogDescription>
                  Sorry, this feature is not available yet. We are working on it.
                </DialogDescription>
              </DialogHeader>
              <DialogFooter className='flex gap-1'>
                ...
                <Button onClick={() => setIsCreatorDialogOpen(false)}  className="bg-[#5956E9] hover:bg-[#4845c7]">
                  Notify Me
                </Button>
              </DialogFooter>
            </DialogContent>
